 ...
 *  d) The data type definition of the sent object: {id:string, from:string, to:string|string[], cmd:string, payload?:any}
 */
...
class JsonRWS {
   ...
   * @returns {{id:string, from:string, to:string|string[], cmd:string, payload?:any}}
   */
  incoming(msgSTR) {
    let tf = false;
    let msg;
  ...
  async processing(msg, socket, dataTransfer, socketStorage, eventEmitter) {
    const { id, from, to, cmd, payload } = msg; // { id, from, to, cmd, payload }
    ...
    if (cmd === 'socket/sendone') {
      ...
      const target_id = msg.to;
      const toSocket = await socketStorage.findOne({ id: target_id });
      ...
      const socketIDs = to; // 'to' is expected to be an array of strings (socket IDs)
    ...
    /*** room commands ***/
      ...
      const roomName = payload;
      socketStorage.roomExit(socket, roomName);
